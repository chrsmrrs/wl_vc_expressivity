...
def main():
    ...
    for d in datasets:
        ...
        for s in range(1, (len(cycles) + 1)):
            acc_train, s_train, acc_test, s_test = gnn_evaluation(GIN, cycles[0:s], d, [1,2,3,4,5], [64], max_num_epochs=200, batch_size=128,
    ...
    for d in datasets:
        ...
        for s in range(1, (len(cliques) + 1)):
            acc_train, s_train, acc_test, s_test = gnn_evaluation(GIN, cliques[0:s], d, [1,2,3,4,5], [64], max_num_epochs=200, batch_size=128,
                                           start_lr=0.01, num_repetitions=num_reps, all_std=False)
            print(acc_train, s_train, acc_test, s_test, acc_train-acc_test)
